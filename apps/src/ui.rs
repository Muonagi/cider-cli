...
/// Print a status/info message: `  › message`
pub fn status(msg: impl Into<Cow<'static, str>>) {
    let prefix = style("›").cyan();
    println!("  {prefix} {}", style(msg.into()).dim());
}
...
/// Print a section header with a leading blank line for visual separation.
pub fn header(msg: impl Into<Cow<'static, str>>) {
    println!();
    println!("  {}", style(msg.into()).bold());
}
...
pub fn progress_bar(total: u64) -> ProgressBar {
    ...
    pb.set_style(
        ProgressStyle::default_bar()
            .template("  {spinner:.dim} Installing [{bar:30.cyan/dim}] {pos}%")
            .unwrap()
            .progress_chars("━╸─"),
    );
    pb.enable_steady_tick(std::time::Duration::from_millis(80));
